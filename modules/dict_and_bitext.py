...
def dict_and_bitext(lemmatizer,unigrams_path,phrases_path):
	...
	for i in df.index:
		...
		if type(df['a'][i]) != float and type(df['b'][i]) != float and type(df['c'][i]) != float:
			...
			t=(lemmatize(lemmatizer,re.sub(ur'( |-|\u002d|\u005f|\u00ad|\u0331|\u0332|\u0335|\u0336|\u2012|\u2013|\u2014|\u2015|\u2017|\u2212|\u2500)+', '_', df['a'][i],re.UNICODE).strip(),df['b'][i].strip()), df['b'][i].strip(), 'def', df['c'][i].strip())
			tl.append(t)
	...
	for i in df.index:
		...
		if type(df['a'][i]) != float and type(df['c'][i]) != float:
			bitext.append(AlignedSent(df['c'][i].split(),df['a'][i].split()))
			t=(lemmatize(lemmatizer,re.sub(ur'( |-|\u002d|\u005f|\u00ad|\u0331|\u0332|\u0335|\u0336|\u2012|\u2013|\u2014|\u2015|\u2017|\u2212|\u2500)+', '_', df['a'][i],re.UNICODE).strip(),'phrase.'), 'phrase.', 'def', df['c'][i].strip())
			tl.append(t) 
	sys.stdout.write("\n")					
